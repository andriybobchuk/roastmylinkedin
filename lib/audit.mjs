...

{
  ...
  "score_breakdown": {
    "dimensions": [
      { "name": "Discoverability",     "score": <0-100>, "commentary": "<1 sentence honest read>" },
      { "name": "Positioning",         "score": <0-100>, "commentary": "<1 sentence honest read>" },
      { "name": "Proof of shipping",   "score": <0-100>, "commentary": "<1 sentence honest read>" },
      { "name": "Seniority signals",   "score": <0-100>, "commentary": "<1 sentence honest read>" },
      { "name": "Content & authority", "score": <0-100>, "commentary": "<1 sentence honest read>" },
      { "name": "Recommendations",     "score": <0-100>, "commentary": "<1 sentence honest read>" }
    ]
  }
}
...
- Score breakdown dimensions must be honest. A weak profile can dip to 30s
  on multiple axes. Total does not need to equal the overall score — these
  are independent signals. NEVER include "fixes" for dimensions here.
...

{
  ...
  "premium_teasers": {
    "interview_preview": {
      "question": "<one behavioral question, e.g. 'Tell me about a time you led through ambiguity'>",
      "answer_start": "<first ~45 words of a STAR answer grounded in a REAL project/company/patent from the profile — stop mid-thought so the reader wants the rest>"
    },
    "recruiter_preview": [
      { "keyword": "<role-relevant search term a recruiter would type>", "currently_visible": true|false },
      { "keyword": "<another role-relevant search term>",                 "currently_visible": true|false }
    ],
    "headline_angles": [
      "Senior-IC frame",
      "Recruiter-keyword frame",
      "Unusual hook",
      "Seniority-signal frame",
...
Rules:
- Use ONLY real data from the profile. Never invent numbers.
- Match the profile's language if not English. No AI slop, no "passionate about", no "results-driven".
- interview_preview.answer_start must reference a real project/company/patent
  from THIS profile and stop before the "Result" of STAR — the tease is the point.
- recruiter_preview: pick two distinct keywords, at least one where the
  profile currently misses, so the free reader sees both outcomes.
- headline_angles: return exactly the five strings listed above, in that
  order. Do not translate them.
